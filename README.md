...
| Генерация | Gemini через Google AI Studio |
...
4. **Хранилища** — Chroma (`vector_db/`), файлы (`data/uploads`, `data/docs`), ключи моделей (`data/models.json`).
...
| Gemini | Генерация только по контексту | Google AI Studio |
...
1. Файл пишется в `data/uploads`.
2. Считается SHA-256. Тот же хеш — файл не переиндексируется.
3. PDF режется по страницам, текст — одним документом.
4. Чанки получают `filename`, `page`, `file_hash`.
5. BGE-M3 считает эмбеддинги на GPU батчами; в списке документов виден прогресс.
6. Запись в Chroma идёт через **один** PersistentClient — повторное открытие той же базы на Windows ломает Chroma 1.5.
...
5. Иначе Gemini отвечает строго по контексту. В карточке — файл, страница, цитата; клик открывает PDF на нужной странице.
...
2. Меню модели → **Подключить API** → ключ Google AI Studio.
...
| 4 | Повторно загрузить тот же файл | Сообщение, что файл уже добавлен; индекс не дублируется. |
...
- загрузка PDF/MD/TXT, чанкинг, Chroma, поиск, генерация Gemini;
...
|----------------------------------------|-------------------------------------------------------------------------------------|------------------------------------------------------|
| Ошибки при работе с примерами из книги | Устаревшие функции в примерах из книги                                              | Замена устаревших методов на обновленные             |
